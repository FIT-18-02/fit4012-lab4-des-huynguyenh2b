...
class DES {
    ...
    public:
        ...
        string encrypt(const string& block) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                string next_right = Xor(perm2, left);
                left = right;
                right = next_right;
            }
        ...
        string decrypt(const string& block) {
            string perm = initial_permutation(block);
            string left = perm.substr(0, 32);
            string right = perm.substr(32, 32);

            // Giải mã dùng round keys theo thứ tự ngược lại (15 -> 0)
            for (int i = 15; i >= 0; i--) {
                string right_expanded = "";
                for (int j = 0; j < 48; j++) {
                    right_expanded += right[expansion_table[j] - 1];
...
// Hàm thêm Zero-padding cho đủ bội số của 64 bit
string pad_plaintext(string pt) {
    while (pt.length() % 64 != 0) {
        pt += "0";
    }
    return pt;
}

// Mã hóa nhiều block (Chế độ ECB)
string encrypt_ecb(DES& des, string pt) {
    pt = pad_plaintext(pt);
    string ct = "";
    for (size_t i = 0; i < pt.length(); i += 64) {
        ct += des.encrypt(pt.substr(i, 64));
    }
    return ct;
}

// Giải mã nhiều block (Chế độ ECB)
string decrypt_ecb(DES& des, string ct) {
    string pt = "";
    for (size_t i = 0; i < ct.length(); i += 64) {
        pt += des.decrypt(ct.substr(i, 64));
    }
    return pt;
}

int main(int argc, char* argv[]) {
    // Trường hợp mặc định: Không có tham số (dùng cho test_sample.sh)
    if (argc == 1) {
        ...
        cout << "Ciphertext: " << des.encrypt(plaintext) << endl;
        return 0;
    }

    // Trường hợp có tham số: ./des <mode> <key> <data>
    if (argc >= 4) {
        string mode = argv[1];
        string key = argv[2];
        string data = argv[3];
        ...
        if (mode == "encrypt") {
            cout << "Result: " << encrypt_ecb(des, data) << endl;
        } else if (mode == "decrypt") {
            cout << "Result: " << decrypt_ecb(des, data) << endl;
        } else {
            cerr << "Error: Unknown mode. Use 'encrypt' or 'decrypt'." << endl;
            return 1;
        }
        return 0;
    }

    cout << "Usage: ./des <encrypt|decrypt> <64-bit key> <binary_data>" << endl;
    return 1;
}